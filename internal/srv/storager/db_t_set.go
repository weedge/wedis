...
func (db *DBSet) sSetItem(key []byte, member []byte) (int64, error) {
	t := db.batch
...
func (db *DBSet) Clear(key []byte) (int64, error) {
	if err := checkKeySize(key); err != nil {
		return 0, err
	}
...
// Mclear clears multi sets.
func (db *DBSet) Mclear(keys ...[]byte) (int64, error) {
	t := db.batch
	t.Lock()
	defer t.Unlock()
